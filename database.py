...
email TEXT, phone INTEGER, house_type TEXT, current_pets TEXT, kids_under_ten TEXT, pet_insurance TEXT, seeking TEXT, matches_id INTEGER, FOREIGN KEY(profile_id) REFERENCES profiles(profile_id), FOREIGN KEY(matches_id) REFERENCES matches(matches_id))"""
...
shelters(shelter_id INTEGER PRIMARY KEY AUTOINCREMENT, profile_id INTEGER, shelter_name TEXT, shlter_address TEXT, shelter_rating INTEGER, pet_id INTEGER,
...
ADD_NEW_USER = """INSERT INTO users (profile_id, first_name, last_name, email, phone, house_type, current_pets, kids_under_ten, pet_insurance, seeking, matches_id) 
VALUES (?,?,?,?,?,?,?,?,?,?,?);"""

ADD_NEW_MATCH = "INSERT INTO matches (user_id, pet_id, shelter_id, match_time) VALUES (?,?,?,?);"

ADD_NEW_SHELTER = "INSERT INTO shelters (profile_id, shelter_name, shelter_address, shelter_rating, pet_id) VALUES (?,?,?,?,?);"
...
def add_user(connection, profile_id, first_name, last_name, email, phone, house_type, current_pets, kids_under_ten, pet_insurance, seeking, matches_id):
    with connection:
        return connection.execute(ADD_NEW_USER, (profile_id, first_name, last_name, email, phone, house_type, current_pets, kids_under_ten, pet_insurance, seeking, matches_id))
...
def add_shelter(connection,profile_id,shelter_name, shelter_address, shelter_rating, pet_id):
    return connection.execute(ADD_NEW_SHELTER, (profile_id,shelter_name, shelter_address, shelter_rating, pet_id))
